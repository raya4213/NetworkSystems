 ...
 * send_image: Transfers the file from Server to Client
 ...
 indexFrmPort: decide by port number used
 filenumber: used for sending the file (as each server has two file parts)
 *****************************************************/
...
int send_image(int sockfd, char *filename, struct sockaddr_in clientAddr, int indexFrmPort, int fileNumber){
  ...
  if (fileNumber == -1)
  {
    ...
    {
      sprintf(systemListCmd, "ls -a DFS%d/%s > DFS%d/%s/.%s", indexFrmPort, putfileUser.USERNAME, indexFrmPort, putfileUser.USERNAME, filename);
    ...
    {
      sprintf(systemListCmd, "ls -a DFS%d/%s/%s > DFS%d/%s/%s/.%s", indexFrmPort, putfileUser.USERNAME, subfolder,indexFrmPort, putfileUser.USERNAME, subfolder,filename);
      printf("systemListCmd %s\n", systemListCmd);
      system(systemListCmd);
      printf("&&&&&&&&&&&&&&&&&&& %s\n", systemListCmd);
    }
  ...
  else
  {
    ...
    if (strcmp(subfolder,"/") == 0)
      //sprintf(dfsFolder,"%s%s/",dfsFolder);
    {
  ...
  char checkFileExists[200];
   ...
   else if (fileNumber>=-1)
   {
      if (strcmp(subfolder,"/") == 0)  
      {
        sprintf(checkFileExists,"DFS%d/%s/%s",indexFrmPort,putfileUser.USERNAME,filenameWithDot);
      }
      else
      {
        printf("comine gere\n");
       sprintf(checkFileExists,"DFS%d/%s/%s/%s",indexFrmPort,putfileUser.USERNAME,subfolder,filenameWithDot); 
      }
...
indexFrmPort: calculated from port number: USED for creating DFS folders
...
/*****************************************************
 * Verifies the user credentils
 * sends proper acknowledgement accordingly
 *****************************************************/
    ...
    switch(option)
    {
      ...
      case LIST_FILES:
        ...
        if (sendAckforUserdetails(newsockfd))
        {
          ...
          printf("The file name asked for first time is %s\n", fileNameList);
          send_image(newsockfd, fileNameList, clientAddr, indexFrmPort, -1); 
          sleep(1);
        }
