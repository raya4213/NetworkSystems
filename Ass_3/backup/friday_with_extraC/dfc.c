...
int receive_image(int sockfd, char *filename, struct sockaddr_in servAddr, socklen_t clientlen, char *subfolder, char *fileNameToUse)
{ 
  ...
  sprintf(fileNameToUse,"%s",filename_received);
  //printf("fileNameToUse %s\n", fileNameToUse);
	...
	while(1)
	{
		...
		switch(option)
		{
			...
			case GET_FILE:
			    ...
			    if ( !sendFailedArray[0] && !sendFailedArray[0])
			    {
			    	printf("FAILED \n");
			    }
			   #if 1
			    for (i = 0; i< MAX_CONNECTIONS; i+2)
			    #else
			    	for (i = 0; i< 1; i++)
			   #endif
			    //tempVali = i;
			    	...
			    	if (!sendFailedArray[i])
			    	{
			    		if (first_time)
						{
				    		if (sendUserDetails(sockfd[i])){
								...
								receive_image(sockfd[i], getFileName, servAddr[i], servlen[i], subfolder, fileNameToUse);
								printf("**********************\n");
								printf("fileNameToUse  %d\n", fileNameToUse[strlen(getFileName)]);
								files_not_received[0] = 3 - (fileNameToUse[strlen(getFileName)] - 48);
								sleep(1);
								// receiving image 2
								n = send(sockfd[i], getFileName, 50, 0);
								if (n < 0)//, (struct sockaddr *)&servAddr, sizeof(servAddr))==-1)
								...
								printf("**********************\n");
								printf("fileNameToUse  %d\n", fileNameToUse[strlen(getFileName)]);
								files_not_received[1] = 3 - (fileNameToUse[strlen(getFileName)] - 48);
			    ...
			    {
			    	char catCommand[300];
			    	sprintf(catCommand,"cat %s %s %s %s > %s_received", fileList[0],fileList[1],fileList[2],fileList[3], getFileName);
			    	//printf("wow %s\n", catCommand);
			    	system(catCommand);
