...
int main(int argc, char **argv)
{
  ...
  printf("indexFrmPort %d\n", indexFrmPort);
    ...
    switch(option)
    {
      ...
      case LIST_FILES:
        // n = recv(newsockfd, (void *)&listDummy, sizeof(int), 0);    // Reading the option from user side
        // if (n < 0)
        // {
        //   perror("option receiving failed");
        //   exit(1);
        ...
        //   char FinalFile[100];
        //   bzero(findFileName, sizeof(findFileName));
        //   sprintf(findFileName,"ls DFS%d/%s",indexFrmPort,putfileUser.USERNAME);
        //   //strncpy(systemmd5Cmd, "md5sum ", sizeof("md5sum ")); 
        //   //strncat(systemmd5Cmd, filename, strlen(filename));
        //   FILE *f = popen(findFileName, "r");
