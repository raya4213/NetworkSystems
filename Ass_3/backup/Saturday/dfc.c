		...
		switch(option)
		{
			...
			case GET_FILE:
			    ...
			    else
			    {
			    	...
			    	for(i=0;i<MAX_CONNECTIONS;i++)
			    	{
			    		sprintf(decryptSystemCmd,"openssl enc -d -aes-256-cbc -in %s -out de%s -k %s", fileList[i], fileList[i], USERNAME);
			    		printf("mama here%s\n", decryptSystemCmd);
			...
			case PUT_FILE:
			    ...
			    for (i=0;i<MAX_CONNECTIONS;i++)
			    {
			    	sprintf(encryptSystemCmd,"openssl enc -aes-256-cbc -in en%s%d -out %s%d -k %s", putFileName,i, putFileName, i, USERNAME);
				...
				for (i =0; i<MAX_CONNECTIONS;i++)
				{
					...
					{ 
						if (sendUserDetails(sockfd[i]))
			    		{
							...
							//exit(1);
							receive_image(sockfd[i], fileNameList, servAddr[i], servlen[i], subfolder);
							printf("\n\nLOOPPP\n");
						}
						//	exit(1);
					}
