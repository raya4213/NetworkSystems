...
void sendUserDetails(int sockfd1)
{
	int n, ack_putfile;
	...
	while(1)
	{
		...
		switch(option)
		{
			...
			case PUT_FILE:
			    ...
			    {
			    	sendUserDetails(sockfd[i]);
			    	strncpy(filenameWithIndex[i], putFileName, sizeof(putFileName));
			    	sprintf(fileIndex,"%d",i);
			    	printf("fileIndex  :   %s\n", fileIndex);
			    	strncat(filenameWithIndex[i], fileIndex, 1);
			    	printf("filename %s\n", filenameWithIndex[i]);
					send_image(sockfd[i], filenameWithIndex[i], servAddr[i]);
					bzero(filenameWithIndex[i], sizeof(filenameWithIndex[i]));
