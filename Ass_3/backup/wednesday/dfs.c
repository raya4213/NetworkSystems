...
int send_image(int sockfd, char *filename, struct sockaddr_in clientAddr, int indexFrmPort){
  ...
  printf("dfsFolder %s\n", dfsFolder);
  strncat(dfsFolder,"/",sizeof("/")); //dfs folder with slash command
  char filenameWithDot[100];
  bzero(filenameWithDot,sizeof(filenameWithDot));
  #if 1
  
  strcpy(filenameWithDot, name[0]);  // take care over here
...
void sendAckforUserdetails(int newsockfd)
{
    ...
    switch(option)
    {
      ...
      case GET_FILE:
          printf("receiving the file name\n");
              ...
              sendAckforUserdetails(newsockfd);
              stat = recv(newsockfd, getFileName, 50,0);
               if (stat < 0)
               {
                ...
                exit(1);
                  ...
                  send_image(newsockfd, getFileName, clientAddr, indexFrmPort);  
...
//        }
      ...
      case PUT_FILE:
            ...
            // printf("USERNAME %s\n", putfileUser.USERNAME);
            // printf("PASSWORD %s\n", putfileUser.PASSWORD);          
