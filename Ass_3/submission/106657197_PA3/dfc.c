 ...
 * send_image: Transfers the file from Client to Server
 ...
 subfolder the subfolder in folder with userame
 *****************************************************/
...
/*****************************************************
 * creates and binds sockets with MAX_CONNECTIONS
 * Also sets the time out to sockest using setsockopt
 *****************************************************/
...
return : ACK from serber for USER credentals validity
 *****************************************************/
...
int main(int argc, char **argv)
{
    ...
    if (fp == NULL)
    {
  	...
  	// ignnores the signal generated due to failure of send: helps in running the code continuously withoy termination
	...
	while(1)
	{
		...
		// also collect information of which servers are active:VERY IMPORTANT
		for (i=0;i<MAX_CONNECTIONS;i++)
		{
			printf("**Option Sent !!\n");
		...
		switch(option)
		{
			...
			case GET_FILE:
				...
				// optimized the get functionality
				// receives files from sever 0 and 2 by default
					// if 0 FAIL receives from 1,2,3
				// else from 1 and 3
					//if 1 FAIL incomplete
				...
				// receives files based on the 	serverToUse[] array constructed as described above 
			   #if 1
			    for (i = 0; i< MAX_CONNECTIONS; i++)
			    #else
			    	for (i = 0; i< 1; i++)
				...
				// resoring the serverToUse values: VERY IMPORTANT
				for (i = 0;i<MAX_CONNECTIONS;i++)
				{
					serverToUse[i] = -1;
				}
